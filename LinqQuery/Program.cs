...
List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Danielle W. Grier", DateTime.Parse("October 18, 1953"), "321-456-089", "DanielleWGrier@rhyta.com", "1973 Stoneybrook Road Maitland, FL 32751", 3599),
            ...
            new Debtor("Dseomhy J. lroin", DateTime.Parse("July 9, 1971"), "270-456-3288", "DorothyJBrown@rhyta.com", "699 Harper Street Louisville, KY 40202", 7956),
...
//---2---
//debtors.Where(d => d.Email.Contains("rhtha.com") || d.Email.Contains("dayrep.com")).ToList().ForEach(d => Console.WriteLine(d));
////---3---
//DateTime date = DateTime.Now;

//debtors.Where(d=> (date.Year - d.BirthDay.Year)>26 && (date.Year - d.BirthDay.Year)<36).ToList().ForEach(d=>Console.WriteLine(d));
//---4---
//debtors.Where(d => d.Debt <= 5000).ToList().ForEach(d => Console.WriteLine(d));
//---5---
//debtors.Where(d => d.FullName.Count() >= 18 && d.Phone.Contains('7')).Count() >=2).ToList().ForEach(d => Console.WriteLine(d));
//---6---
//debtors.Where(d=>d.BirthDay.Month == 1 || d.BirthDay.Month == 2 || d.BirthDay.Month ==12).ToList().ForEach(d=>Console.WriteLine(d));
...
//debtors.Where(d => d.Debt > average()).ToList().ForEach(d => Console.WriteLine(d));

//8
//var sortedList = debtors.OrderBy(d =>d.Debt).OrderBy(d => d.FullName).ToList();

//sortedList.ForEach(d => Console.WriteLine(d));

//---9---

//debtors.Where(d => d.Phone.Contains("8") == false).ToList().ForEach(d => Console.WriteLine(d.Debt));
//---11---

//var filterDebtors = debtors.Where(d => d.FullName.ToLower().Where(char.IsLetter).GroupBy(c =>c).Any(g => g.Count() > 3)).OrderBy(d =>d.FullName).ToList();


//foreach (var filter in filterDebtors)
//{
...
//var maxDebt = debtors[0].Debt;

//int n = 0;

//List<Debtor> list = new List<Debtor>();

//var maxDebtList = (debtors.Take(5).Where(d=>d.Debt >= maxDebt).ToList());
//maxDebtList.ForEach(d => Console.WriteLine(d));
...
//var debtorlist = debtors.Where(d =>HasUniqueDigits(d.Phone) == true).ToList();
...
//bool BirtDayMonth(Debtor debtor)
//{
//    int month = nowDateTime.Month - debtor.BirthDay.Month;
...
//debtors.Where(d =>BirtDayMonth(d)).ToList().ForEach(d=>Console.WriteLine(d));

//---20---
bool ConatainLetter(Debtor debtor)
{
    var txt = "smile";
    List<char>list1 = new List<char>() {'s','m','i','l','e'};
    int count = 0;
    foreach (char c in list1)
    {
        if (debtor.FullName.Contains(c)) { 
            count++;
        };
    }
    if(count == 5)
...
debtors.Where(d=>ConatainLetter(d) == true).ToList().ForEach(d=>Console.WriteLine(d));